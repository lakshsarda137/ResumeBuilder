...
- **Format** follows the Rice CCD guide's sample letter, minus the parts this app skips: **no recipient block, no company address, no "RE:" line**, and a fixed `Dear Hiring Team,`. Header → date → greeting → opening (3-4 sentences) → 1-2 body paragraphs tied back to the JD → closing paragraph with email/phone and an interview ask → `Sincerely,` + typed name. Prose only, one page, ~300-400 words.
- **Voice** (`COVER_LETTER_VOICE_RULES`) bans ~40 AI-tell words (tapestry, delve, passionate, thrilled, eager, seamless, testament…), the four stock openings, claims of feeling, and compliments to the company; requires varied sentence openings (not every sentence starting with "I"); and forbids saying anything about the employer that is not in the job description text. Em dashes are already banned by the shared `EM_DASH_PROMPT_RULE`.
